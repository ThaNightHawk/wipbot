...
namespace wipbot
{
    ...
    class Config
    {
        ...
        [UseConverter(typeof(ListConverter<string>))]
        [NonNullable]
        public virtual List<string> UrlFindReplacePairs { get; set; } = [
            "https://drive.google.com/file/d/",
        ...
        public virtual QueueLimits QueueLimits { get; set; } = new QueueLimits { User = 1, Subscriber = 2, Vip = 3, Moderator = 5 };
    ...
    public class Plugin
    {
        ...
        static List<QueueItem> wipQueue = [];
        ...
        public void OnMessageReceived(String userName, String msg, bool isBroadcaster, bool isModerator, bool isVip, bool isSubscriber)
        {
            ...
            if (msgSplit[0].ToLower().StartsWith(Config.Instance.CommandRequestWip))
            {
                ...
                {
                    string inputArgument = msgSplit[1].Trim();
                    bool isRequestCode = inputArgument.All(Config.Instance.RequestCodeCharacterWhitelist.Contains);
                    bool isWhitelistedUrl = Config.Instance.UrlWhitelist.Any(prefix => inputArgument.StartsWith(prefix));
                    ...
                    else if (isRequestCode)
                    {
                        ...
                        {
                            if (inputArgument.StartsWith(Config.Instance.RequestCodePrefixDownloadUrlPairs[i]))
                            {
                                wipUrl = Config.Instance.RequestCodePrefixDownloadUrlPairs[i + 1].Replace("%s", inputArgument);
                                break;
                            }
                        }
                        if (wipUrl == "")
